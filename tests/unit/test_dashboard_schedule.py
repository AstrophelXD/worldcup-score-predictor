...
def test_team_flags_cover_all_group_teams():
    ...
    for team in teams:
        ...
        assert "flagcdn.com" in url
        assert team in team_label_plain(team)


def test_team_id_resolves_to_country_flag():
    assert team_flag_image_url("team_bra").endswith("/br.png")
    assert team_flag_image_url("team_arg").endswith("/ar.png")
...
def test_team_label_html_renders_image_tag():
    ...
    assert "<img" in html
    assert "gb-eng.png" in html
    assert "England" in html


def test_scotland_uses_subdivision_flag():
    assert team_flag("Scotland").endswith("/gb-sct.png")
